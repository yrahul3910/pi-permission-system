...
function hasSubagentEnvHint(): boolean {
  const values = SUBAGENT_ENV_HINT_KEYS.map(
...
type PermissionPromptDetails = PermissionRequestCommonFields & {
  ...
  bashEvaluation?: BashEvaluation | null;
...
const FILESYSTEM_TOOL_NAME_SUFFIXES = [
  "read",
  "write",
  "edit",
  "find",
...
function buildDesktopNotificationBody(message: string): string {
  const firstLine = message.split(/\r?\n/, 1)[0]?.trim() ?? "";
  const body = firstLine || "A tool call is waiting for your approval.";
  return body.length > DESKTOP_NOTIFICATION_BODY_MAX_LENGTH
    ? `${body.slice(0, DESKTOP_NOTIFICATION_BODY_MAX_LENGTH - 1)}…`
    : body;
}

/**
 * Fires a desktop notification for a waiting permission prompt when the user is
 * not looking at this terminal tab. We only suppress the notification when we
 * have positively observed that the tab is focused; if focus reporting is
 * unavailable (e.g. tmux without `focus-events on`), we still notify so the user
 * is not left waiting silently.
 */
function notifyPermissionWaitingIfUnfocused(message: string): void {
  if (!extensionConfig.desktopNotifications) {
    return;
  }

  if (
    focusTracker &&
    focusTracker.hasObservedFocusEvents() &&
  ...
  try {
    sendDesktopNotification(
      ...
      buildDesktopNotificationBody(message),
    );
  } catch {
    // Notifications are best-effort; never let them interrupt a permission flow.
  }
}
...
function isLikelyFilesystemToolName(toolName: string): boolean {
  ...
  return FILESYSTEM_TOOL_NAME_SUFFIXES.some(
    (suffix) => normalized.endsWith(suffix) || nameParts.includes(suffix),
  );
}
...
function getPathBearingToolPath(
  ...
  if (
    hasStructuredEditPayload(inputRecord) ||
    isLikelyFilesystemToolName(toolName)
  ) {
    return path;
  }
...
function isFinalAssistantMessage(message: unknown): boolean {
  ...
  return (
    ...
    !record.content.some(
      (content: unknown) => toRecord(content).type === "toolCall",
    )
  );
}
...
function extractSkillNameUnderRoot(
  normalizedReadPath: string,
  normalizedSkillsRoot: string,
): string | null {
  ...
  const skillName = relativeSkillPath
    .split(/[\\/]+/)
...
function inferSkillEntryFromReadPath(
  ...
  for (const skillRoot of candidateSkillRoots) {
    ...
    const skillName = extractSkillNameUnderRoot(
      normalizedReadPath,
      normalizedSkillRoot,
    );
    if (!skillName) {
...
function createSensitiveLogMetadata(
  value: string | undefined,
): SensitiveLogMetadata | null {
  if (value === undefined) {
    return null;
  }
...
function clonePermissionPromptDecision(
...
function createDuplicatePermissionPromptDecision(
  decision: PermissionPromptDecision,
): PermissionPromptDecision {
...
function pickPermissionPromptToolFields(
...
function createPermissionPromptFingerprint(
  details: PermissionPromptDetails,
): string {
  return (
    safeJsonStringify({
...
function createPermissionPromptCacheKey(
  details: PermissionPromptDetails,
): string | null {
...
function getPatternApprovalSubject(
  result: PermissionCheckResult,
  input: unknown,
): string {
...
function createConfigEvaluationRule(
  result: PermissionCheckResult,
): PatternPermissionRule {
  const canReuseMatchedPattern =
    result.source === "bash" ||
    result.source === "mcp" ||
    result.source === "skill" ||
  ...
  return {
    ...
    pattern:
      canReuseMatchedPattern && result.matchedPattern
...
function applyPatternApprovalState(
  ...
  recheckBashWithSession?: (sessionAllowPrefixes: readonly string[][]) => PermissionCheckResult,
...
function getPermissionDecisionScope(details: {
  ...
  return (
    getNonEmptyString(details.target) ??
...
function persistSessionApprovalDecision(
  ...
  if (decision.state === "always_family") {
    // Session family approval. The families are re-derived from the real
    // evaluation of the command text — never from a UI label or a forwarded
    ...
    const stored = sessionApprovals.approveBashFamilyPrefixes(families);
    return stored.length > 0 ? { subject: stored.join(", "), persistence: "session" } : null;
  }
...
function waitForForwardedPermissionResponseFile(
  responsePath: string,
  deadline: number,
): Promise<void> {
  ...
  return new Promise((resolve) => {
    ...
    try {
      watcher = watch(
        responseDir,
        { persistent: false },
        (_eventType: unknown, fileName: unknown) => {
...
function resolvePathWithinDirectory(
  directory: string,
  fileName: string,
): string | null {
  ...
  return isPathWithinDirectory(comparablePath, comparableDirectory)
    ? resolvedPath
    : null;
}
...
function logPermissionForwardingEntry(
  event: "warning" | "error",
  message: string,
  error?: unknown,
): void {
...
function setRestrictiveFileSystemMode(
  path: string,
  mode: number,
  description: string,
): void {
  try {
    ...
    logPermissionForwardingWarning(
      `Failed to restrict ${description} permissions for '${path}'`,
      error,
    );
  }
}
...
function getPermissionForwardingLocationForSession(
  ...
  return createPermissionForwardingLocation(
    getPermissionForwardingRootDir(ctx),
    sessionId,
  );
}
...
function isForwardedPermissionResponseBoundToRequest(
  ...
  if (
    ...
    !safeEqualString(response.responseNonce, request.responseNonce)
  ) {
    logPermissionForwardingWarning(
      `Ignoring forwarded permission response '${responsePath}' because it is not bound to request '${request.id}'`,
    );
    return false;
  }
  if (response.responderSessionId !== (responderSessionId ?? targetSessionId)) {
    logPermissionForwardingWarning(
      `Ignoring forwarded permission response '${responsePath}' because responder session '${response.responderSessionId}' does not match target session '${targetSessionId}'`,
    );
    return false;
  }
  return true;
...
function writeJsonFileAtomic(filePath: string, value: unknown): void {
  ...
  try {
    ...
    setRestrictiveFileSystemMode(
      tempPath,
      0o600,
      "temporary permission-forwarding file",
    );
...
async function readForwardedPermissionResponse(
  filePath: string,
  ...
  while (Date.now() < deadline) {
    if (existsSync(responsePath)) {
      ...
      writeReviewEntry("forwarded_permission.response_received", {
        ...
        denialReasonMetadata: createSensitiveLogMetadata(
          boundResponse?.denialReason,
        ),
...
export async function processForwardedPermissionRequests(
  ...
  for (const { requestPath, request } of pendingRequests) {
    ...
    const responsePath = resolvePathWithinDirectory(
      location.responsesDir,
      `${request.id}.json`,
    );
    if (!responsePath) {
      logPermissionForwardingWarning(
        `Ignoring forwarded permission request '${request.id}' because its response path would escape '${location.responsesDir}'`,
      );
      safeDeleteFile(
        requestPath,
        `${location.label} forwarded permission request`,
    ...
    // A replacement owner may have taken over while this dialog was pending.
    if (options.signal?.aborted) return;
...
export default function piPermissionSystemExtension(pi: ExtensionAPI): void {
  ...
  let permissionForwardingController = new AbortController();
  ...
  let permissionManager = createPermissionManagerForCwd(
    undefined,
    notifyWarning,
  );
  const sessionApprovals = new SessionApprovalStore();
  const recentPermissionPromptDecisions = new Map<
    string,
    CachedPermissionPromptDecision
  >();
  ...
  const saveExtensionConfig = (
    ...
    // Apply the in-memory config even when the disk write fails: yolo mode is
    // session-scoped and needs no persistence, so a session-local yolo toggle
    // must not be silently dropped just because writing the synced fields
    ...
    if (!saved.success) {
      ...
      writeDebugEntry("config.save_failed", {
  ...
  const setYoloModeFromRuntimeApi = (
    enabled: boolean,
  ...
  // Standalone sessions expose their API before session_start. In-process
  // children load after the interactive parent has claimed ownership.
  ...
  const ensureModelOptionCompatibilityRegistered = (): Promise<void> => {
    if (!modelOptionCompatibilityRegistration) {
      modelOptionCompatibilityRegistration = import(
        "./model-option-compatibility.js"
      )
        .then(
          ({
            registerModelOptionCompatibilityGuard,
          }: {
            registerModelOptionCompatibilityGuard: (pi: ExtensionAPI) => void;
          }) => {
  ...
  pi.registerCommand("permission-system", {
    ...
    handler: createPermissionSystemCommandHandler(async (ctx) => {
      const { openPermissionSystemSettingsModal } = await import(
        "./config-modal.js"
      );
  ...
  const reviewPermissionDecision = (
    ...
    writeReviewEntry(event, {
      ...
      bashEvaluation: details.bashEvaluation ?? null,
      target: details.target ?? null,
      toolInput: details.toolInput ?? null,
  ...
  const promptPermission = async (
    ...
    const cachedDecision = cacheKey
      ...
      : null;
    if (cachedDecision) {
      const decision = createDuplicatePermissionPromptDecision(
    ...
    const decisionPromise = (async (): Promise<PermissionPromptDecision> => {
      ...
      // When this session owns the UI, alert the user off-tab that a prompt is
      // waiting. Subagent sessions (no UI) forward to the parent, which fires its
      // own notification in processForwardedPermissionRequests, so we skip here to
    ...
    if (cacheKey) {
      rememberPermissionPromptDecision(
        recentPermissionPromptDecisions,
        cacheKey,
        decisionPromise,
      );
    ...
    try {
      ...
      if (cacheKey) {
        forgetPermissionPromptDecision(
          recentPermissionPromptDecisions,
          cacheKey,
          decisionPromise,
        );
  ...
  const closePermissionForwardingWatcher = (): void => {
    if (permissionForwardingWatcher) {
      try {
        ...
        logPermissionForwardingWarning(
          "Failed to close permission forwarding watcher",
          error,
        );
      }
  ...
  const runForwardedPermissionRequestScan = (): void => {
    if (!runtimeApi || interactiveRuntime.api !== runtimeApi) {
    ...
    const signal = permissionForwardingController.signal;
  ...
  const queueForwardedPermissionRequestScan = (): void => {
    if (
      ...
      permissionForwardingScanTimer ||
      permissionForwardingController.signal.aborted
    ) {
      return;
    }
  ...
  const startPermissionForwardingFallbackTimer = (): void => {
    if (
      permissionForwardingFallbackTimer ||
      permissionForwardingController.signal.aborted
    ) {
      return;
    }
  ...
  const startForwardedPermissionPolling = (
    ...
    if (claimOwnership) {
      interactiveRuntime.stopForwarding?.();
      interactiveRuntime.api = runtimeApi;
      interactiveRuntime.stopForwarding = stopForwardedPermissionPolling;
      registerPiPermissionSystemRuntimeApi(runtimeApi);
    }
    if (permissionForwardingController.signal.aborted) {
      permissionForwardingController = new AbortController();
    }
    ...
    try {
      permissionForwardingWatcher = watch(
        location.requestsDir,
        { persistent: false },
        (_eventType: unknown, fileName: unknown) => {
          ...
          if (
            ...
            normalizedFileName.endsWith(".tmp")
          ) {
            queueForwardedPermissionRequestScan();
          }
        },
  ...
  const shouldExposeTool = (
    ...
    const toolPermission = applyPatternApprovalState(
      {
        toolName,
        state: permissionManager.getToolPermission(
          toolName,
          agentName ?? undefined,
        ),
        source: "tool",
    ...
    if (
      toolName === "read" &&
      permissionManager.hasAllowedSkills(agentName ?? undefined)
    ) {
      return true;
    }
  ...
  const ensureFocusTracker = (ctx: ExtensionContext): void => {
    // Focus reporting relies on raw terminal input, which is only available in
    // interactive TUI mode. Start the tracker once per interactive session.
    if (focusTracker || !ctx.hasUI || ctx.mode === "rpc") {
      return;
    }

    const onTerminalInput = ctx.ui.onTerminalInput?.bind(ctx.ui);
    if (typeof onTerminalInput !== "function") {
      return;
    }

    focusTracker = startTerminalFocusTracker({ onTerminalInput });
  };

  const disposeFocusTracker = (): void => {
    if (focusTracker) {
  ...
  pi.on("session_shutdown", async () => {
    ...
    if (interactiveRuntime.api === runtimeApi) {
      ...
      interactiveRuntime.stopForwarding = null;
    }
  ...
  pi.on("tool_call", async (event: ToolCallEvent, ctx: ExtensionContext) => {
      ...
      if (readPath) {
        ...
        const inferredSkillCheck = inferredSkill
          ? permissionManager.checkPermission(
              "skill",
              { name: inferredSkill.name },
              agentName ?? undefined,
            )
          : null;
        const readSkill =
          matchedSkill ??
          (inferredSkill && inferredSkillCheck
        ...
        if (readSkill && !explicitlyRequestedSkillNames.has(readSkill.name)) {
          ...
          if (readSkill.state === "ask") {
            ...
            if (!decision.approved) {
              ...
              return {
                block: true,
                reason: `User denied access to this skill.${denialReason}`,
              };
            }
          }
        }
    ...
    if (
      ...
      const extCheck =
        rawExtCheck.state === "ask"
          ? applyPatternApprovalState(
              rawExtCheck,
              externalPermissionInput,
              sessionApprovals,
            )
    ...
    if (check.state === "ask") {
      ...
      const approval = persistSessionApprovalDecision(
        decision,
        check,
        input,
        sessionApprovals,
