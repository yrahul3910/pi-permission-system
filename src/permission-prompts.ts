...
export function getStructuredEditPayloads(
...
export function hasStructuredEditPayload(
  inputRecord: Record<string, unknown>,
): boolean {
 ...
 */
export function formatBashEvaluationNotice(
  evaluation: BashEvaluation | undefined,
): string | null {
  if (
    !evaluation ||
    evaluation.state === "allow" ||
    evaluation.pieces.length === 0
  ) {
    return null;
  }
  ...
  const lines = shown.map((piece) => {
    ...
    const display = truncateInlineText(
      piece.display.replace(/\s+/g, " ").trim(),
      BASH_PIECE_DISPLAY_MAX_LENGTH,
    );
...
function formatPermissionHardStopHint(result: PermissionCheckResult): string {
  if ((result.source === "mcp" || result.toolName === "mcp") && result.target) {
    return "This MCP permission denial is policy-enforced. Do not retry this target.";
  }

  return "This permission denial is policy-enforced. Do not retry or investigate bypasses.";
}
...
export function formatExternalDirectoryHardStopHint(): string {
  return "This external directory permission denial is policy-enforced. Do not retry this path and do not attempt a filesystem bypass.";
}
