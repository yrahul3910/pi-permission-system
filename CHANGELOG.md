...
- Reworked bash approval prompts to list **exactly the blocking pieces** with plain-language reasons, and generalized session approvals to `Allow for this session: <families>` (e.g. `wc`, `git push`) — per-piece prefix approvals that also silence matching pieces of later compound commands, safe by construction because every piece is still evaluated individually.
