...
The background tool's `working_dir` is resolved against the session directory before checking commands. Both directories are then resolved to their real filesystem paths for containment checks and relative command-path evaluation. Calls with a missing or unresolvable directory are blocked; neither the process directory nor a caller-supplied `cwd` can substitute for the session directory. A directory outside the session, including the target of an in-project symlink, must also pass `special.external_directory`.

Permission prompts identify `bg_start`, the command, and its working directory, including when a tool-level policy requires approval. "Allow Always" approves that exact background command in that resolved working directory for the session, including when only the tool policy required approval. A different directory requires its own approval; symlink aliases of the approved directory share the approval. It does not approve every background command in the directory, and wildcard characters in the command remain literal approval text. Session-approved command families are intentionally shared between `bash` and `bg_start`; both use the same bash policy, while background tool and protected-path restrictions still apply.
...
The timeout covers the entire forwarded request, including time waiting for the parent to display it. With the timeout off (`"forwardedPromptTimeoutSeconds": null`), the request waits until answered. A request keeps the deadline selected when it was created; setting changes apply to new requests. YOLO in the receiving parent auto-approves forwarded `ask` requests without showing a countdown. YOLO does not override a `deny` returned by policy evaluation; the [file-tool exceptions](#file-tool-policy-exceptions) can skip normal tool/path evaluation. Each in-process session owns its YOLO state. The interactive parent's API and forwarding session ID are registered process-wide, so they survive fresh extension-module imports when a worktree changes Pi's loader cache. Starting or shutting down a child cannot replace that registration; only the current owner can clear it. A successful handoff stops the previous poller, closes its forwarded dialogs, and discards late results from its in-flight scans. With YOLO off, recognized children still forward asks to the parent's dialog. Sessions without subagent metadata do not acquire parent permissions merely by sharing a process.
